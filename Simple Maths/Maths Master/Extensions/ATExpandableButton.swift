...
class ATExpandableButton: UIView {
  ...
  private func setup() {
    ...
    originalLabelFrame = CGRect(x: 5, y: 0, width: frame.width - 10, height: frame.height)
  ...
  private func createTitleLabel() -> UILabel {
    let label = UILabel()
    label.text = title
    label.font = UIFont(name: "AvenirNext-DemiBold", size: 26.0)
  ...
  func expand() {
    ...
    setupExpandedView()
    hideOthers()
    expandedChildView.awakeFromNib()
  ...
  private func moveOrHideTitleAndIcon(expanding: Bool) {
    if keepsTitleWhenExpanded {
      let width = originalLabelFrame.width
      titleLabel.frame = expanding ? CGRect(x: (frame.width / 2) - (width / 2), y: 10, width: width, height: 40) : originalLabelFrame
    } else {
